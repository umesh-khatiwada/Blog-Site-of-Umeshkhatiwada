...
export default function DevOpsSidebar({
  ...
  const [categories, setCategories] = useState<SubCategory[]>(
    initialCategoryData.data.sub_categories || []
  );
  const [fullCategory, setFullCategory] = useState<Category | null>(
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      if (
        categoryId === "0" ||
        categoryId.toString() === initialCategoryData.data.id.toString()
      ) {
        return;
      }
